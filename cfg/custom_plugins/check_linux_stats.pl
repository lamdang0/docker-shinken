...
sub check_cpu {
        ...
        if(defined($stat->cpustats)) {
                ...
                print " $status CPU : Used $cpu_used%  $perfdata";
        }
        else {
                print "No data";
        }
...
sub check_mem {
        ...
        if(defined($stat->memstats)) {
                ...
                my $totalserver = sprintf("%.0f", ($mem->{memtotal}/1024));
                my $memused_GB = sprintf("%.0f", ($mem->{memused}/1024));
                my $memcached = sprintf("%.2f", ($mem->{cached}/$mem->{memtotal})*100)||0;
                my $memused = sprintf("%.0f", (($mem->{memused} - $mem->{cached})/$mem->{memtotal})*100);
                ...
                print "$status MEMORY : Total = ${totalserver}Mb, Used = ${memused_GB}Mb - $memused% used  $perfdata";
        }
        else {
                print "No data";
        }
...
sub check_net {
        ...
        if(defined($stat->netstats)) {
                ...
                foreach my $device (keys (%$net)) {
                        my $txbyt = sprintf("%.0f", $net->{$device}->{txbyt});
                        my $rxerrs = sprintf("%.0f", $net->{$device}->{rxerrs});
                        my $ttbyt = sprintf("%.0f", $net->{$device}->{ttbyt});
                        my $txerrs = sprintf("%.0f", $net->{$device}->{txerrs});
                        my $txdrop = sprintf("%.0f", $net->{$device}->{txdrop});
                        my $txcolls = sprintf("%.0f", $net->{$device}->{txcolls});
                        my $rxbyt = sprintf("%.0f", $net->{$device}->{rxbyt});
                        my $rxdrop = sprintf("%.0f", $net->{$device}->{rxdrop});
                        $tx = bytes_to_readable($txbyt);
                        $rx = bytes_to_readable($rxbyt);

                         if(defined($checkthis->{$device})||defined($checkthis->{all})){
                                if($ttbyt>=$o_critical){ $crit++;}
                                elsif($ttbyt>=$o_warning){ $warn++;}

                                $return_str .= bytes_to_readable($ttbyt)." ";
                ...
                print "$status USAGE : Total = $return_str, txbyt = $tx, rxbyt = $rx $perfdata";
        }
}
...
sub check_load {
        ...
        if(defined($stat->loadavg)) {
                ...
                print "$status LOAD AVERAGE : $load->{avg_1},$load->{avg_5},$load->{avg_15} $perfdata";
        }
        else {
                print "No data";
        }
